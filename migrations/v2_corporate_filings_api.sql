-- Migration: V2 Corporate Filings API
-- Creates user_read_filings table and RPC function for the authenticated corporate filings endpoint
...
CREATE POLICY "Service role full access on user_read_filings"
    ON user_read_filings
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- ============================================================
-- 2. RPC function: get_corporate_filings_v2
--    Handles all filtering: watchlist, marketcap, read/unread,
--    categories, symbols, ISINs, duplicates, date range, pagination.
--    Returns filings with investorCorp data and is_read flag.
-- ============================================================
CREATE OR REPLACE FUNCTION get_corporate_filings_v2(
    ...
    p_read_filter TEXT DEFAULT 'all',        -- 'all', 'read', 'unread'
    p_marketcap TEXT[] DEFAULT NULL,          -- array of: 'large','mid','small','micro','nano'
...
BEGIN
    ...
    v_total_pages := GREATEST(1, CEIL(v_total_count::FLOAT / p_page_size)::INTEGER);

    -- Get paginated filings with investorCorp data
