...
import { Phone, RefreshCcw, ArrowLeft } from "lucide-react";

export default function ConcallTranscriptsPage() {
  const { token } = useAuth();
    ...
    <div className="flex flex-col h-full">
      <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base md:text-lg font-bold text-gray-900">Concall Transcripts</h1>
          <span className="text-xs text-gray-400 ml-2">{totalCount.toLocaleString()} transcripts</span>
        </div>
        ...
        <div className={`flex-1 overflow-hidden ${selected ? "flex flex-col" : "hidden md:flex md:flex-col"}`}>
          {selected ? <AnnouncementDetail filing={selected} /> : (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              <Phone size={32} className="mb-3 text-gray-300" />
              <p className="text-sm">Select a transcript to view details</p>
            </div>
          )}
        </div>
      </div>
